...
<%@ page import="teamhalp.dao.*, java.util.*, java.sql.*"%>
...
<meta charset="UTF-8">
	...
	<!--  Sources for the bootstraps used:
			 	Main bootstrap
				...
				upload picture
				http://bootsnipp.com/snippets/66Ex7
    ...
    <title>HALPful Blogs HomePage</title>
...
<!-- Navigation bar  - Displays a different navigation bar depending on whether a user is logged on -->
		...
		<div class="row">
			<div class="col-md-8">
				...
				if (isSearch || isSorted) {
					...
					if (articles == null) {
						problems += "Cannot find any articles.\n";
					}	
				// else, get all articles
				} else {
					ArticleDAO accessor = new ArticleDAO();
					articles = accessor.getAll();
					title = "Latest Posts";
					if (articles==null) {
						problems+= "Error accessing database. Site may be down. Please consult administrators.\n";
					}
				}%>
				...
				<%  int startIndex = 0;	//start Index of this page (taking page parameter into account later on)
					...
					for(int i = startIndex; i <= endIndex; i++) {
						...
						author = article.getCreated_by();
						int article_id = article.getArticle_id();
						// Get number of comments
						List<Comment> comments = new CommentDAO().getByArticle_id(article_id);
						int numberOfComments  = (comments == null? 0 : comments.size());%>
					<article>
					<h3>
						<a href="Article.jsp?article=<%=article_id%>"><%=article.getTitle()%></a>
					</h3>
	
					<div class="row">
						<div class="col-sm-6 col-md-6">&nbsp;&nbsp;<span class="glyphicon glyphicon-bookmark"></span>
							Created by <a href="UserHome.jsp?author=<%=author%>" name="author"><%=author%></a></div>
				...
				<%}%> <!-- End for loop for article generation -->
				...
				<ul class="pager"> 
					...
					<%if(request.getParameter("page")!=null){
						...
						if (shouldDisplayPrevious) {%>	
							<li class="previous"><a href="Homepage.jsp?page=<%=(pageParameter-1) + searchAndSortParameters%>">&larr; Previous</a></li>
						<%}%>
					<%}%>
				...
				if (shouldDisplayNext) {%>	
					<li class="next"><a href="Homepage.jsp?page=<%=(pageParameter+1) + searchAndSortParameters%>">&rarr; Next </a></li>
				<%}%>
				</ul>
			...
			<div class="col-md-4">
                ...
                <div class="well">
                    ...
                    <form name="sortArticles" action="RequestArticle" method="get">
                     <% if (isSearch) {%>
                     	<input type="hidden" name="alreadySearch" value="search"/>
                     <%}%>
                     <select title="Select your search criteria" class="selectpicker" name="doSort" required >
                        <option selected disabled value=""></option>
					 	...
					 	<option value="author-asc">Author &#x25B2</option>
					  	<option value="author-desc">Author &#x25BC</option>
